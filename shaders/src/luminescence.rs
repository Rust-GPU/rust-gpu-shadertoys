...
impl State {
  fn map(&self, mut p: Vec3, id: Vec3) -> De {
    ...
    if p.y < 0.5 {
      ...
      let mut d3: f32 = (mp.xz() - vec2(0.2, 0.1)).length() - remap(0.5, -3.5, 0.1, 0.01, mp.y);
      if d3 < o.d {
        o.m = 2.;
      }
      d3 += ((mp.y * 10.0).sin() + (mp.y * 23.0).sin()) * 0.03;

      let d32: f32 = (mp.xz() - vec2(0.2, 0.1)).length() - remap(0.5, -3.5, 0.1, 0.04, mp.y) * 0.5;
      d3 = d3.min(d32);
      o.d = smin(o.d, d3, 0.5);
      ...
      if p.y < 0.2 {
        ...
        let d4: f32 = (op.xz() - vec2(0.85, 0.0)).length() - remap(0.5, -3.0, 0.04, 0.0, op.y);
        if d4 < o.d {
          o.m = 3.0;
        }
  ...
  fn vol_tex(&self, uv: Vec3, mut p: Vec3, scale: f32, pump: f32) -> f32 {
    ...
    s = s * s;
    s *=
      smoothstep(1.4, -0.3, uv.y - (s2 * TWO_PI).cos() * 0.2 + 0.3) * smoothstep(-0.6, -0.3, uv.y);
      ...
      if o.m == 1.0 {
        ...
        for i in 0..VOLUME_STEPS {
          ...
          if sd != MAX_DISTANCE {
            ...
            let uv: Vec3 = vec3(intersect.x.atan2(intersect.y), intersect.length(), o.uv.z);
