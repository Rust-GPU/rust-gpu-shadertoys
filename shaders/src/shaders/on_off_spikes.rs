...
const SUN_POS: Vec3 = vec3(15.0, 15.0, -15.0);
const SUN_SPHERE: bool = false;

const SPHERE_COL: Vec3 = vec3(0.6, 0.3, 0.1);
const MOUTH_COL: Vec3 = vec3(0.9, 0.6, 0.1);
const TENTACLE_COL: Vec3 = vec3(0.06, 0.06, 0.06);
...
impl State {
  fn scene(&self, p: Vec3) -> Hit {
    ...
    if BACKGROUND {
      q = p;
      q = q.yz().rem_euclid(Vec2::ONE).extend(q.x).zxy();
      q -= vec3(-0.6, 0.5, 0.5);
  ...
  pub fn main_image(&mut self, frag_color: &mut Vec4, frag_coord: Vec2) {
    ...
    if HARD_SHADOW {
      ...
      col -= Vec3::splat(0.24 * smoothstep(0.5, -0.3, dist.min(sun_dist) / sun_dist.max(0.0001)));
    }
