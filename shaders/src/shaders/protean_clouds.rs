...
impl State {
  fn map(&self, mut p: Vec3) -> Vec2 {
    ...
    p = (rot((p.z + self.inputs.time).sin() * (0.1 + self.prm1 * 0.05) + self.inputs.time * 0.09)
      .transpose()
      * p.xy())
    .extend(p.z);
  ...
  fn render(&self, ro: Vec3, rd: Vec3, time: f32) -> Vec4 {
    ...
    for _ in 0..130 {
      ...
      if mpv.x > 0.6 {
        col = ((vec3(5.0, 0.4, 0.2) + Vec3::splat(mpv.y * 0.1 + (pos.z * 0.4).sin() * 0.5 + 1.8))
          .sin()
          * 0.5
          + Vec3::splat(0.5))
        .extend(0.08);
        ...
        let mut dif: f32 = ((den - self.map(pos + Vec3::splat(0.8)).x) / 9.0).clamp(0.001, 1.0);
        dif += ((den - self.map(pos + Vec3::splat(0.35)).x) / 2.5).clamp(0.001, 1.0);
        col = (col.xyz() * den * (vec3(0.005, 0.045, 0.075) + 1.5 * vec3(0.033, 0.07, 0.03) * dif))
          .extend(col.w);
      }
...
impl State {
  pub fn main_image(&mut self, frag_color: &mut Vec4, frag_coord: Vec2) {
    ...
    let target: Vec3 = (ro - (disp(time + tgt_dst) * dsp_amp).extend(time + tgt_dst)).normalize();
    ro.x -= self.bs_mo.x * 2.;
