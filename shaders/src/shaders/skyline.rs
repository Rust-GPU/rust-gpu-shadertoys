...
impl<C0> State<C0> {
  ...
  fn get_env_map_skyline(&self, ray_dir: Vec3, sun_dir: Vec3, height: f32) -> Vec3 {
    ...
    let mut skyline: f32 =
      (((5.3456 * radial).sin() + (1.234 * radial).sin() + (2.177 * radial).sin()) * 0.6).floor();
    radial *= 4.0;
    ...
    if (t <= max_depth) || (t == alpha) {
      ...
      if (normal.y < 0.1) && (dist_and_mat.y == 0.0) {
...

        {
          ...
          sidewalk *= saturate(((pos.z * block_size.x).sin() * 800.0 / block_size.x).abs());
          sidewalk *= saturate(((pos.x * block_size.y).sin() * 800.0 / block_size.y).abs());
      ...
      light_color += (self.sky_col * saturate(normal.y * 0.5 + 0.5)) * ambient_avg.powf(0.35) * 2.5;
      light_color *= 4.0;
  ...
  pub fn main_image(&mut self, frag_color: &mut Vec4, frag_coord: Vec2) {
    ...
    if NON_REALTIME_HQ_RENDER {
      for _ in 0..ANTIALIASING_SAMPLES {
        ...
        self.local_time +=
          hash11(v21(frag_coord + Vec2::splat(self.seed))) * motion_blur_length_in_seconds;
