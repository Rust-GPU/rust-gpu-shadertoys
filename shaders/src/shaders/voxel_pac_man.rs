  ...
  if HSV2RGB_FAST {
    hsv.z
      * (Vec3::ONE
        + 0.5
          * hsv.y
          * ((2.0 * PI * (Vec3::splat(hsv.x) + vec3(0.0, 2.0 / 3.0, 1.0 / 3.0))).cos() - Vec3::ONE))
  } else {
    hsv.z
      * (Vec3::ONE
        + Vec3::splat(hsv.y)
...
impl State {
  ...
  pub fn main_image(&mut self, frag_color: &mut Vec4, frag_coord: Vec2) {
    // Get the fragment
    let frag: Vec2 = (2.0 * frag_coord - self.inputs.resolution.xy()) / self.inputs.resolution.y;
    ...
    let mode_switch: f32 =
      smoothstep(0.995, 1.0, mode_timing) + smoothstep(0.02, 0.0, mode_angle.abs()) * mode_voxel;
